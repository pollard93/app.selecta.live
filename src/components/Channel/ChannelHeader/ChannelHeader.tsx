...
const ChannelHeader: FC<ChannelHeaderProps> = (props) => {
   ...
   */
  const coverImageHeadingHeight = useMemo(() => scrollY.current.interpolate({
    inputRange: [-1000, 0, coverImageHeadingDefaultHeight.current],
    outputRange: [1000 + coverImageHeadingDefaultHeight.current, coverImageHeadingDefaultHeight.current, 0],
    extrapolate: 'clamp',
  }), []);
  ...
  const titlePaddingTop = useMemo(() => {
    ...
    return scrollY.current.interpolate({
      inputRange: [0, coverImageHeadingDefaultHeight.current, coverImageHeadingDefaultHeight.current + titleLayout.height],
  ...
  const headerTopHeight = useMemo(() => {
    if (headerTopLayout.height === 0 || titleLayout.height === 0) return null;

    return scrollY.current.interpolate({
      inputRange: [0, coverImageHeadingDefaultHeight.current, coverImageHeadingDefaultHeight.current + titleLayout.height],
      outputRange: [headerTopLayout.height, headerTopLayout.height, titleLayout.height],
      extrapolate: 'clamp',
    });
  }, [headerTopLayout.height, titleLayout.height]);
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <H3 disableBaseColor>{props.data.name}</H3>
            </Animated.Text>
          </Animated.View>
        </View>
      </Animated.View>
